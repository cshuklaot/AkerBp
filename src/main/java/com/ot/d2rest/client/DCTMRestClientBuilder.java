...
public final class DCTMRestClientBuilder {
	...
	public static DCTMRestClient buildWithPrompt() {
		...
		String bindingStr = read(sb.toString(), "json");
		DCTMRestClientBinding binding = DCTMRestClientBinding.valueOf(bindingStr.toUpperCase());
		String contextRoot = read("Please input the REST context path:", "http://localhost:8080/d2rest");
		...
		String password = read("Please input the password:", "demo.demo");
		boolean useFormatExtension = read("Please input the whether add format extension .xml or .json for URI:",
				false);
		String debug = read("Please input whether print debug information:", "true");
		AbstractRestTemplateClient client = (AbstractRestTemplateClient) new DCTMRestClientBuilder().bind(binding)
				.contextRoot(contextRoot).ignoreAuthenticateServer(ignoreAuthenticateServer)
				.ignoreSslWarning(ignoreSslWarning).credentials(username, password).repository(repository)
				.useFormatExtension(useFormatExtension).debug("true".equalsIgnoreCase(debug)).build();
		client.getHomeDocument();
		if (client.getMajorVersion() >= 7.2) {
			String msg = client.getMajorVersion() >= 7.3
					? "Please input whether enable the client token authentication with the server based CSRF:"
					: "Please input whether enable the client token authentication:";
	 ...
	 * @return the DCTMRestClient
	 */
	public static DCTMRestClient buildSilently(DCTMRestClientBinding binding, String contextRoot, String repository,
	 ...
	 */
	public static DCTMRestClient buildSilently(DCTMRestClientBinding binding, String contextRoot, String repository,
			String username, String password, boolean useFormatExtension, boolean debug,
			boolean enableCSRFClientToken) {
		boolean ignoreAuthenticateServer = ignoreAuthenticateServer(contextRoot);
		AbstractRestTemplateClient client = (AbstractRestTemplateClient) new DCTMRestClientBuilder().bind(binding)
				.contextRoot(contextRoot).ignoreAuthenticateServer(ignoreAuthenticateServer)
				.credentials(username, password).repository(repository).useFormatExtension(useFormatExtension)
				.debug(debug).build();
		client.getHomeDocument();
		if (client.getMajorVersion() >= 7.2) {
			client.enableCSRFClientToken(enableCSRFClientToken);
	...
	public static DCTMRestClient buildAsyncClient(DCTMRestClient client) {
		...
		return (DCTMRestClient) Proxy.newProxyInstance(DCTMRestClient.class.getClassLoader(),
				new Class[] { DCTMRestClient.class }, new ClientAsyncRestTemplateClient(100, client));
	}

	private static List<String> getRepositories(DCTMRestClientBinding binding, String contextRoot,
			boolean ignoreAuthenticateServer, boolean ignoreSslWarning) {
	...
	/**
	 * the context root the REST service e.g. http://localhost:8080/dctm-rest
	 * 
	 * @param contextRoot
	 *            the contextRoot of the rest
	...
	/**
	 * Ignore SSL certificate warning even when the certificate is untrusted. This
	 * should be used only in test environment.
	 * 
	 * @param ignoreSslWarning
	 *            boolean to indicate ignoring the ssl warning
	 * @return
	 */
	...
	/**
	 * whether append .xml or .json extension to the uri e.g.
	 * http://localhost:8080/dctm-rest/services.xml or
	 * http://localhost:8080/dctm-rest/services.json
	 * 
	 * @param useFormatExtension
	 *            whether use format extension
	 * @return the DCTMRestClientBuilder itself
	 */
	...
	/**
	 * @param ignoreAuthenticateServer
	 *            whether ignore authenticate the server for ssl
	 * @return the DCTMRestClientBuilder itself
	 */
