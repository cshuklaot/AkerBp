 ...
 */
package com.ot.akbp.rest.client.sample.client.impl;

import static com.ot.akbp.commons.util.rest.model.LinkRelation.BATCHES;
import static com.ot.akbp.commons.util.rest.model.LinkRelation.EDIT;
import static com.ot.akbp.commons.util.rest.model.LinkRelation.PAGING_FIRST;
import static com.ot.akbp.commons.util.rest.model.LinkRelation.PAGING_LAST;
import static com.ot.akbp.commons.util.rest.model.LinkRelation.PAGING_NEXT;
import static com.ot.akbp.commons.util.rest.model.LinkRelation.PAGING_PREV;
import static com.ot.akbp.commons.util.rest.model.LinkRelation.SELF;
import static com.ot.akbp.rest.client.sample.client.util.Headers.ACCEPT_ATOM_HEADERS;
import static com.ot.akbp.rest.client.sample.client.util.Headers.ACCEPT_JSON_HEADERS;
import static com.ot.akbp.rest.client.sample.client.util.Headers.ACCEPT_JSON_HEADERS_WITH_CONTENT;
import static com.ot.akbp.rest.client.sample.client.util.Headers.ACCEPT_JSON_HEADERS_WITH_MULTIPART_CONTENT;
import static com.ot.akbp.rest.client.sample.client.util.Headers.ACCEPT_XML_HEADERS;
import static com.ot.akbp.rest.client.sample.client.util.Headers.ACCEPT_XML_HEADERS_WITH_CONTENT;
import static com.ot.akbp.rest.client.sample.client.util.Headers.ACCEPT_XML_HEADERS_WITH_MULTIPART_CONTENT;
import static com.ot.akbp.rest.client.sample.client.util.Headers.JSON_CONTENT;
import static com.ot.akbp.rest.client.sample.client.util.Headers.XML_CONTENT;
...
import com.ot.akbp.commons.util.rest.model.FutureModel;
...
import com.ot.akbp.rest.client.batch.Attachment;
import com.ot.akbp.rest.client.batch.Batch;
import com.ot.akbp.rest.client.batch.Operation;
import com.ot.akbp.rest.client.sample.client.converter.MultipartBatchHttpMessageConverter;
import com.ot.akbp.rest.client.sample.client.util.DCContentUploader;
import com.ot.akbp.rest.client.sample.client.util.Debug;
import com.ot.akbp.rest.client.sample.client.util.Headers;
import com.ot.akbp.rest.client.sample.client.util.Randoms;
import com.ot.akbp.rest.client.sample.client.util.Strings;
import com.ot.akbp.rest.client.sample.client.util.SupportedMediaTypes;
import com.ot.akbp.rest.client.sample.client.util.UriHelper;
...
public abstract class AbstractRestTemplateClient implements DCTMRestClient {
	...
	/**
	 * HomeDocument resource uri is the only fixed uri it must be: "Rest context
	 * path"/services e.g. http://localhost:8080/dctm-rest/services
	 * 
	...
	private HttpHeaders setupRequestHeader(HttpHeaders headers) {
		...
		if (enableCSRFClientToken && clientToken != null) {
			...
			// the cookie is already processed by HttpComponentsClientHttpRequestFactory,
			// need not to set client token header
			...
			result.set(HttpHeaders.AUTHORIZATION,
					"Basic " + new String(Base64.encodeBase64(usernameAndPassword.getBytes())));
			if (debug) {
				Debug.debug(
						"Authenticate with Basic " + new String(Base64.encodeBase64(usernameAndPassword.getBytes())));
			}
		}
		if (ifMatch != null) {
	...
	protected <T> T sendRequest(String uri, HttpMethod httpMethod, HttpHeaders headers, Object requestBody,
			Class<T> responseBodyClass, String... params) {
		...
		HttpEntity<Object> requestEntity = requestBody == null ? new HttpEntity<Object>(headers)
				: new HttpEntity<Object>(
						requestBody instanceof FutureModel ? ((FutureModel) requestBody).getModel() : requestBody,
						headers);
	...
	public void uploadDistributedContent(String url, InputStream content) {
		try {
			...
			if (uploader.getStatus() != HttpStatus.OK.value() && uploader.getStatus() != HttpStatus.FOUND.value()) {
				...
				error = jerror;
				Debug.error(error);
				throw new DCTMRestErrorException(uploader.getHeaders(), HttpStatus.valueOf(uploader.getStatus()),
						error);
			}
		} catch (IOException e) {
	...
	protected <T> T get(String uri, boolean isCollection, Class<? extends T> responseBodyClass, String... params) {
		return get(uri, isXml() ? (isCollection ? ACCEPT_ATOM_HEADERS : ACCEPT_XML_HEADERS) : ACCEPT_JSON_HEADERS,
					...
					if (response.getStatusCode() != HttpStatus.OK) {
						...
						if (tmp.isDirectory()) {
							String filename = Strings
									.getFilename(response.getHeaders().getFirst(HttpHeaders.CONTENT_DISPOSITION));
							if (filename == null) {
							...
							String ext = Strings
									.getFilenameExt(response.getHeaders().getFirst(HttpHeaders.CONTENT_DISPOSITION));
							if (ext != null) {
	...
	protected <T> T post(String uri, Object body, Class<? extends T> responseBodyClass, String... params) {
		return sendRequest(uri, POST, isXml() ? ACCEPT_XML_HEADERS_WITH_CONTENT : ACCEPT_JSON_HEADERS_WITH_CONTENT,
				body, responseBodyClass, params);
	}

	protected <T> T post(String uri, Object body, HttpHeaders headers, Class<? extends T> responseBodyClass,
			String... params) {
		return sendRequest(uri, POST, headers, body, responseBodyClass, params);
	}

	protected <T> T post(String uri, Object content, String contentMediaType, Class<? extends T> responseBodyClass,
			String... params) {
		return sendRequest(uri, POST,
				new Headers()
						...
						.contentType(contentMediaType).toHttpHeaders(),
	...
	protected <T> T put(String uri, Object body, Class<? extends T> responseBodyClass, String... params) {
		return sendRequest(uri, PUT, isXml() ? ACCEPT_XML_HEADERS_WITH_CONTENT : ACCEPT_JSON_HEADERS_WITH_CONTENT, body,
				responseBodyClass, params);
	}
	...
	protected <T> T post(String uri, T object, Object content, String contentMediaType,
		...
		if (content == null) {
			...
			t = post(uri, content,
					contentMediaType == null ? MediaType.APPLICATION_OCTET_STREAM_VALUE : contentMediaType,
					responseBodyClass, params);
		} else {
			MultiValueMap<String, Object> parts = new LinkedMultiValueMap<String, Object>();
			parts.add("metadata", new HttpEntity<Object>(object, isXml() ? XML_CONTENT : JSON_CONTENT));
			parts.add("binary", contentMediaType == null ? content
					: new HttpEntity<Object>(content, new Headers().contentType(contentMediaType).toHttpHeaders()));
			t = sendRequest(uri, POST,
					isXml() ? ACCEPT_XML_HEADERS_WITH_MULTIPART_CONTENT : ACCEPT_JSON_HEADERS_WITH_MULTIPART_CONTENT,
					parts, responseBodyClass, params);
		}
		return t;
	}

	protected <T> T post(String uri, T object, List<Object> contents, List<String> contentMediaTypes,
			Class<? extends T> responseBodyClass, String... params) {
		T t = null;
		if (contents == null || contents.isEmpty()) {
			t = post(uri, object, responseBodyClass, params);
		} else if (contents.size() == 1) {
			t = post(uri, object, contents.get(0),
					contentMediaTypes != null && contentMediaTypes.size() > 0 ? contentMediaTypes.get(0) : null,
					responseBodyClass, params);
		} else {
			MultiValueMap<String, Object> parts = new LinkedMultiValueMap<String, Object>();
			if (object != null) {
			...
			for (int i = 0; i < contents.size(); ++i) {
				String contentMediaType = (contentMediaTypes != null && i < contentMediaTypes.size())
						? contentMediaTypes.get(i)
						: null;
				parts.add("binary",
						contentMediaType == null ? contents.get(i)
								: new HttpEntity<Object>(contents.get(i),
										new Headers().contentType(contentMediaType).toHttpHeaders()));
			}
			t = sendRequest(uri, POST,
					isXml() ? ACCEPT_XML_HEADERS_WITH_MULTIPART_CONTENT : ACCEPT_JSON_HEADERS_WITH_MULTIPART_CONTENT,
					parts, responseBodyClass, params);
	...
	protected Batch post(Batch batch, Class<? extends Batch> responseBodyClass) {
		...
		if (batch.hasAttachment()) {
			...
			HttpHeaders batchHeaders = new Headers()
					.contentType(isXml()
							? String.format("%s;type=\"%s\"", SupportedMediaTypes.APPLICATION_XOP_STRING,
									SupportedMediaTypes.APPLICATION_VND_DCTM_XML_VALUE)
							: String.format("%s;type=\"%s\"", SupportedMediaTypes.APPLICATION_JOP_STRING,
									SupportedMediaTypes.APPLICATION_VND_DCTM_JSON_VALUE))
					.header("Content-ID", "batch").header("Content-disposition", "form-data; name=batch")
					.toHttpHeaders();
	...
	private void addAttachment(MultiValueMap<String, HttpEntity<InputStream>> parts, Attachment attachment) {
		HttpHeaders opHeaders = new Headers().contentType(attachment.getContentType())
				...
				.toHttpHeaders();
		parts.add(attachment.getInclude().getRawHref(),
				new HttpEntity<InputStream>(attachment.getContentStream(), opHeaders));
	}
	...
	public interface RequestProcessor {
		<T> ResponseEntity<T> process(String url, HttpMethod method, HttpEntity<?> requestEntity,
				Class<T> responseType);
	}
		...
		if (restTemplate.getRequestFactory() instanceof HttpComponentsClientHttpRequestFactory) {
			try {
				SSLContext sslContext = new SSLContextBuilder().loadTrustMaterial(null, new TrustSelfSignedStrategy())
						.build();
				X509HostnameVerifier verifier = ignoreSslWarning ? new AllowAllHostnameVerifier()
			...
			Debug.error("the request factory " + restTemplate.getRequestFactory().getClass().getName()
					+ " does not support ignoreAuthenticateServer");
		}
		return this;
	}
	...
	private class DefaultRequestProcessor implements RequestProcessor {
		@Override
		public <T> ResponseEntity<T> process(String url, HttpMethod method, HttpEntity<?> requestEntity,
				Class<T> responseType) {
	...
	protected RestObject getCabinet(String cabinet, Class<? extends RestObject> clazz, String... params) {
		...
		if (entries != null) {
			for (Entry<RestObject> e : entries) {
	...
	public <T extends Linkable> T get(T t, String... params) {
		return get(t.self(), t instanceof FeedBase, getModelClass(t), params);
	}
